...
from .generate import ProfileGenerate
import json
...
class ProfileReport(object):
    ...
    def __init__(self, profile, sample):
        ...
        # description_set = ProfileGenerate.profilegen(df, df_desc,title, bins=10, sample=5, corr_reject=0.9, config={}, **kwargs)
        # Persist the Dictionary for current Dataframe to its respective dbfs location
        # sample =
        self.html = to_html(sample,
                            profile)

        self.description_set = profile
    ...
    def to_file(self, title, outputfile=DEFAULT_OUTPUTFILE):
        if outputfile != NO_OUTPUTFILE:
            ...
            rtitle = title.split("~")[0] + " " + title.split("~")[1] + " Data Profile " + title.split("~")[2] + "(" + title.split("~")[3] + ")"
            self.file.write(self.rendered_html().replace("!Ttl!", rtitle))
            self.file.close()
