...
def describe(df, df_desc,bins, corr_reject, config, **kwargs):
    ...
    def describe_integer_1d(df, column, current_result, nrows):
        if spark_version == "1.6+":
            stats_df = df.select(column).na.drop().agg(mean(col(column)).alias("mean"),
                                                       ...
                                                       count(when(col(column) == 0, True)).alias('n_zeros')
                                                       ).toPandas()
        else:
            stats_df = df.select(column).na.drop().agg(mean(col(column)).alias("mean"),
                                                       df_min(col(column)).alias("min"),
        ...
        stats["iqr"] = round(stats[pretty_name(0.75)] - stats[pretty_name(0.25)],1)
        stats["cv"] = round((stats["std"] / float(stats["mean"])) if stats["mean"] != 0 else 0, 1)
        stats["mad"] = round((df.select(column)
                        .na.drop()
                        .select(df_abs(col(column)-stats["mean"]).alias("delta"))
                        .agg(df_sum(col("delta"))).toPandas().iloc[0,0] / float(current_result["count"])),1)
        stats["type"] = "NUM"
    ...
    def describe_float_1d(df, column, current_result, nrows):
        if spark_version == "1.6+":
            stats_df_0 = df.select(column).na.drop().agg(mean(col(column)).alias("mean"),
                                                       df_min(col(column)).alias("min"),
                                                       df_max(col(column)).alias("max"),
                                                       variance(col(column)).alias("variance"),
                                                       kurtosis(col(column)).alias("kurtosis"),
            ...
            for c_name, c_type in stats_df_0.dtypes:
                if c_type in ('double', 'float'):
                    stats_df_0 = stats_df_0.withColumn(c_name, F.round(c_name, 1))
            stats_df = stats_df_0.toPandas()
        else:
            stats_df = df.select(column).na.drop().agg(mean(col(column)).alias("mean"),
                                                       df_min(col(column)).alias("min"),
                                                       df_max(col(column)).alias("max"),
            ...
            stats_df["variance"] = round(df.select(column).na.drop().agg(variance_custom(col(column),
                                                                                   stats_df["mean"].iloc[0],
                                                                                   current_result["count"])).toPandas().iloc[0][0],2)
            stats_df["std"] = np.sqrt(stats_df["variance"])
            stats_df["skewness"] = df.select(column).na.drop().agg(skewness_custom(col(column),
                                                                                   stats_df["mean"].iloc[0],
                                                                                   current_result["count"])).toPandas().iloc[0][0]
...
#         stats["unparsed_json_types"] = unparsed_valid_jsons
    ...
    def describe_1d(df, df_desc,column, nrows, lookup_config=None):
        column_type = df.select(column).dtypes[0][1]
        # Adding description of each column from df_desc ( Taken care later )
        # df_join_desc=df.join(df_desc,df_desc.Column == column,"inner")
        # desc=df_join_desc.select("Description").take(1)[0][0]
...
#                                  count(col(column).isNotNull()).alias('count')).toPandas()
        ...
        results_data["p_fill_rate"] = results_data["count"] / float(nrows)
    ...
    for colum in df.columns:
        if colum in config:
            if 'lookup' in config[colum]:
                ...
                desc = describe_1d(df, df_desc,colum, table_stats["n"])
        else:
            desc = describe_1d(df, df_desc,colum, table_stats["n"])
    ...
    # Creating a Dictionary of df_desc DF
    # df_desc.toPandas().set_index('Column').T.to_dict('list')
    # #Preparing dataframe from variable_stats to be joined same format as df_desc.toPandas()
    # vstat_t=variable_stats.T
    # # Adding a new column 'Column' from index of existing DF
    # vstat_t['Column'] = vstat_t.index
    # # Performing the join
    # variable_stats_desc = pd.merge(vstat_t,df_desc.toPandas(),how='left', left_on=['Column'], right_on=['Column']).T
    ...
    table_stats["total_fill_rate"] = 1 - float(variable_stats.loc["n_missing"].sum()) / (table_stats["n"] * table_stats["nvar"])
    memsize = 0
    ...
    # variable_stats = variable_stats.applymap(lambda x: "{:,}".format(x) if isinstance(x, float)  else x)
